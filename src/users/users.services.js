...
const postUser = (req, res) => {
  ...
  userControllers
    .createUser({ firstName, lastName, email, password, gender, birthday })
    .then(async (data) => {
...
const patchUser = (req, res) => {
  const id = req.params.id;
  const { firstName, lastName, email, gender, birthday, role, status } =
    req.body;
  ...
  userControllers
    ...
    .then((data) => {
      if (data) {
        res
          .status(200)
          .json({ message: `User edited succesfully with id: ${id}` });
      } else {
...
const patchMyUser = (req, res) => {
  ...
  userControllers
    .updateUser(id, { firstName, lastName, gender, birthday })
    .then(() => {
